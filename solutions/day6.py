...
def find_guard_path_v2(maze: list, start_x: int, start_y: int, direction: str):
    ...
    while (
        x < len(maze) - 1 and y < len(maze[0]) - 1 and (x > 0 and y > 0) and not cycle
    ):
...
def cycles(
    maze: list, start_x: int, start_y: int, start_direction: str, positions: set
):
    total_cycles = 0
...
def print_maze(
    maze_print: list, x: int, y: int, direction: str, cycles: int = 0
) -> None:
...
def part1(filename: str) -> int:
    ...
    total_distinct_positions, _ = find_guard_path_v2(
        maze, start_x, start_y, start_direction
    )
